package fr.eno.starlight.world.gen.feature;
...
public class CustomOreFeature extends Feature<OreFeatureConfig>
{
								...
								for (int j2 = l; j2 <= k1; ++j2)
								{
									...
									if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D)
									{
										...
										if (!bitset.get(k2))
										{
											...
											if (this.getStateList().contains(worldIn.getBlockState(blockpos$mutable).getBlock()))
											{
	...
	protected List<Block> getStateList()
	{
		final List<Block> allBlocks = new ArrayList<Block>(ForgeRegistries.BLOCKS.getValues());
		allBlocks.removeIf((block) -> (block instanceof AirBlock));
		return allBlocks;
	}
}
